...
class LePyMoPDF(FPDF):
    ...
    def __init__(self, image_src, image_scaled, start_date):
        ...
        super().__init__()
        self._abort = 0
        self.pdf_width = _PDF_FORMATS["A4"]["pdf_width"]
        self.pdf_height = _PDF_FORMATS["A4"]["pdf_height"]
    ...
    def main_page(self):
        ...
        self.add_page()
        y_pos = _PDF_FORMATS["A4"]["page_y_pos"]
        ...
        for color, qty in self.colors.items():
            if self._abort == 1:
                return
            if y_pos + _PDF_FORMATS["A4"]["small_brick_margin"] > _PDF_FORMATS["A4"]["page_max_y_pos"]:
                self.add_page()
                y_pos = _PDF_FORMATS["A4"]["medium_header_margin"]
            self.small_brick(_PDF_FORMATS["A4"]["small_brick_x_pos"], y_pos - _PDF_FORMATS["A4"]["small_brick_margin"] / 2, color)
            self.brick_text(f"x {qty} {color}", _PDF_FORMATS["A4"]["small_brick_text_x_pos"], y_pos)
            y_pos += _PDF_FORMATS["A4"]["small_brick_margin"]

        y_pos += _PDF_FORMATS["A4"]["medium_header_margin"]
        if y_pos + _PDF_FORMATS["A4"]["medium_header_font_size"] > _PDF_FORMATS["A4"]["page_max_y_pos"]:
            self.add_page()
            y_pos = _PDF_FORMATS["A4"]["page_y_pos"]
    ...
    def build_step(self, step):
        ...
        y_pos = _PDF_FORMATS["A4"]["page_y_pos"] + _PDF_FORMATS["A4"]["small_header_margin"]
    ...
    def footer(self):
        ...
        if self.page_no() > 1:
            ...
            self.text((self.pdf_width - self.get_string_width(current_page)) / 2, _PDF_FORMATS["A4"]["paging_y_pos"], current_page)

    def brick_text(self, text, position_x, position_y):
        """Adds a brick text to the page"""
        self.set_font('Arial', '', _PDF_FORMATS["A4"]["small_brick_font_size"])
